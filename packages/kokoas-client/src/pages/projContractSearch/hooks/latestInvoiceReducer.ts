...
export const latestInvoiceReducer = (estimateId: string) =>
  (acc: IInvoices, cur: IInvoices) => {
    ...
    const isInvoiceOfEstimateId = cur
      .estimateLists
      .value
      .some(({ value: inv  }) => inv.estimateId.value === estimateId);


    if (isInvoiceOfEstimateId
    && !acc?.issuedDateTime?.value
    || acc?.issuedDateTime?.value < cur.issuedDateTime.value
    ) {
